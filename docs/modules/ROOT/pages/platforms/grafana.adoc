...
----
{
  ...
  "targets": [
    {
      "expr": "(sum by(cluster,db)(redis_server_used_memory{cluster=\"$cluster\", db=\"$db\"}) / avg by(cluster,db)(db_memory_limit_bytes{cluster=\"$cluster\", db=\"$db\"}) / avg by(cluster,db)(db_replication_factor{cluster=\"$cluster\", db=\"$db\"})) * 100",
      "refId": "A"
    }
  ],
  "type": "gauge",
      ...
      - uid: redis-high-memory
        ...
        data:
          - refId: A
            ...
            model:
              expr: (sum by(cluster,db)(redis_server_used_memory) / avg by(cluster,db)(db_memory_limit_bytes) / avg by(cluster,db)(db_replication_factor)) * 100 > 90
              intervalMs: 1000
              maxDataPoints: 43200
        noDataState: NoData
        execErrState: Error
