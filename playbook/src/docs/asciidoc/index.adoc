...
Included in the Redis Observability package is a suite of alerts and their associated tests for use with Prometheus. There are alerts for all of the metrics described in both the Resources and the Performance sections below.
...
There are tests included with the alerts that validate the individual triggers. These can be used to validate any modifications made when tailoring the alerts for specfic environments and/or use cases.
