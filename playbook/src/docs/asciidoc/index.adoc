...
|round(((sum by(cluster,db)(redis_server_used_memory) / avg by(cluster,db)(db_memory_limit_bytes)) / avg by(cluster,db)(db_replication_factor)) * 100) > 98

|The database in question will be unable to accept new data in two hours
|round(((sum by(cluster,db)(redis_server_used_memory) / avg by(cluster,db)(db_memory_limit_bytes)) / avg by(cluster,db)(db_replication_factor)) * 100) < 98 and ((sum by(cluster,db)(predict_linear(redis_server_used_memory[15m], 2 * 3600)) / avg by(cluster,db)(db_memory_limit_bytes)) / avg by(cluster,db)(db_replication_factor)) > 0.3 and round(((sum by(cluster,db)(predict_linear(redis_server_used_memory[15m], 2 * 3600)) / avg by(cluster,db)(db_memory_limit_bytes)) / avg by(cluster,db)(db_replication_factor)) * 100) > 98
