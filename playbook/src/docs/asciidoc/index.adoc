...
[In progress]
- Discuss read vs write vs other latency
- Max latency

Note that this latency metric does not include network round trip time or application-level serialization,
...
|===
|*Issues*|*Investigation*|*Remediation*
...
Certain operations, such as https://redis.io/docs/latest/commands/keys/[KEYS], should never be issued against a
production system due to the impact they have on performance.
...
=======
and https://redis.io/docs/latest/commands/lrem/[LREM]. In addition, customer Lua scripts can also be
computationally expensive.

You can review slow operations from the Redis Enterprise and Redis Cloud UI:

.Cloud console slow log display
image::images/slow_log.png[]

You can also review slow operations from the Redis CLI. For example, here's the command
to review the 100 most recent slow operations:
